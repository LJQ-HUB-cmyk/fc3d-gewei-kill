# -*- coding: utf-8 -*-
"""
福彩3D 个位杀一码 — 生成固定静态网页「个位杀一码.html」
=========================================================
...
BODY_TEMPLATE = """
  <div class="card">
    <div class="kill-box">
      <div class="kill-label" id="killLabel">下期个位杀一码</div>
  ...
  <div class="card">
    <h3>🎛 参数网格扫描 <span style="color:#9ca3af;font-weight:400">(270组合·双段稳健选优 Top36展示)</span></h3>
  ...
  <div class="footer">
    <b>说明</b><br>
    ① 个位杀一码 = 预测杀掉 0-9 中一个数字，下期<b>个位</b>不出现即命中，理论随机基线 <b>90%</b>。<br>
    ② 公式池 <b id="fc">-</b> 个暴力穷举算法（<span id="nfeat">-</span>特征线性组合），在<b>最新500期</b>按命中率选 Top400 专家池（按族限选保证多样性），主机制 <b>Hedge 加权投票</b>：每期取近 <span id="pWin">-</span> 期命中率 Top<span id="pK">-</span> 专家，按命中率加权投票，票王 = 个位杀码。参数经 <b>270 组合双段网格扫描</b>自动选优（段内500期 + 前500期样本外稳健性：先保段内最优、再挑样本外最稳，防选择偏差峰值）。<br>
    ③ 回测为<b>逐期真实预测记录</b>：第 t 期预测只用第 t-1、t-2 期数据（walk-forward，不偷看未来）。<br>
    ④ <b>选择偏差警示</b>：专家池是在回测的同一段 500 期上按命中率选出的——段内高分是选择偏差上界（270组合中挑峰值），真实水平更接近「样本外稳健性」指标（前500期独立段回测）；「固定公式」高分同为<b>选择偏差假象</b>。<b>不构成任何购彩建议</b>。<br>
...
<div class="wrap">
  <div class="topbar">
    <div>
      <div class="t">福彩3D · <b>个位杀一码</b></div>
...
function render(d){{
  ...
  $("killLabel").textContent = n.target_issue + " 期 个位杀一码";
...
def main():
    ...
    print(f"下一期 {n['target_issue']} 个位杀 {n['kill']}")
