...
def main():
    t0 = time.time()
    ap = argparse.ArgumentParser(description='福彩3D 个位杀一码 一键更新')
    ap.add_argument('--force', action='store_true', help='强制重算（忽略缓存）')
    args = ap.parse_args()

    print("=" * 46)
    print("  福彩3D 个位杀一码 · 一键更新")
    ...
    if os.path.exists('cache/pool.json') and not args.force:
        with open('cache/pool.json', 'r', encoding='utf-8') as f:
            pj = json.load(f)
        pfp = f"{pj['data_info']['n_issues']}_{pj['data_info']['last']}_{pj.get('feat_version', '')}"
    ...
    print("本地预览: http://127.0.0.1:8897/个位杀一码.html  (双击 HTML 文件亦可)")
