# -*- coding: utf-8 -*-
"""
福彩3D 个位杀一码 — Hedge 加权投票核心引擎
=============================================
机制（学习 D:\\通杀一码\\core.py 的 hedge_kill）：
  专家池 = 3931万公式在最新500期穷举选出的 TopK（按族限选，见 bruteforce500.py）
  每期预测：近 win 期专家命中率 = 权重(下限 SMOOTH) → 各专家对当期投票 → 票王 = 个位杀码
  参数 win/k 由 270 组合网格扫描在 500 期回测上自动选优。
...
def build_matrices(issues, hh, tt, oo, pool):
    ...
    # 被预测个位：末列占位（预测期，仅用于结构一致，不参与回测）
    at_ext = np.concatenate([np.asarray(oo[L0:N], dtype=np.int16), [0]])  # (651,)
    K = len(pool)
    ...
    return pred, hit, L0, np.asarray(oo, dtype=np.int16)
...
def build_oos_matrices(issues, hh, tt, oo, pool):
    ...
    at_ext = np.concatenate([np.asarray(oo[L0:o_start + WINDOW], dtype=np.int16), [0]])
    K = len(pool)
...
def grid_scan(hit, pred, oo_arr, L0, hit_o=None, pred_o=None, L0_o=None,
...
def next_prediction(pool, pred, hit, L0, issues, hh, tt, oo, fixed_info, best_win, best_k):
    """下一期（2026223）个位杀码：最近 best_win 期专家命中率加权投票。"""
    N = len(hh)
...
def main():
    ...
    scan, best = grid_scan(hit, pred, oo_arr, L0, hit_o=hit_o, pred_o=pred_o, L0_o=L0_o)
