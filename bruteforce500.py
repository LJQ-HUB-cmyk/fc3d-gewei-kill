# -*- coding: utf-8 -*-
"""
福彩3D 个位杀一码 — 暴力穷举（最新500期，只算个位）+ TopK 专家池
=====================================================================
...
def search_pool(hh, tt, oo, window=WINDOW, topk=TOPK, pfl=PFL, verbose=True):
    """3931万公式 × 最新window期穷举（只算个位），按族限选在线维护专家池。
    ...
    start = N - window
    if verbose:
        print(f"穷举窗口: 数据第 {start+1}..{N} 期，共 {window} 期（只算个位）")
    ...
    ah = np.array(oo[start:start + window], dtype=np.int16)   # 被预测的500个个位
